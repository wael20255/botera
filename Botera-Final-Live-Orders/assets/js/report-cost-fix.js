(function(){
  ...
  const inRange=(d,r)=>window.DateRange?.within?DateRange.within(d,r):true;
  ...
  async function run(){
    ...
    const adPerOrder=orderCount?adSpend/orderCount:0;
    ...
    const afterShipping=orderCount?(realizedBase+adSpend)/orderCount:0;
    ...
    root.querySelectorAll('.metric-card').forEach(card=>{
      ...
      if(label==='التكلفة'||label==='تكلفة الأوردر قبل الشحن'||label==='تكلفة الاوردر قبل الشحن'){
        labelEl.textContent='تكلفة الأوردر قبل الشحن';valueEl.textContent=money(adPerOrder,currency);
