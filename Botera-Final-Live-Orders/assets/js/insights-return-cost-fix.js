(async function(){
  ...
  function installReportStyle(){
    ...
    style.textContent=`
      ...
      #reportsMetrics .metric-card:hover{
        transform:translateY(-2px);
        border-color:var(--color-border-strong);
        box-shadow:0 18px 42px rgba(0,0,0,.24);
      }
      #reportsMetrics .metric-card.featured{
        grid-column:span 2;
        min-height:164px;
        padding:24px;
      }
      ...
      @media(max-width:900px){
        #reportsMetrics{grid-template-columns:repeat(2,minmax(0,1fr));}
        #reportsMetrics .metric-card.featured{grid-column:span 1;}
      }
      @media(max-width:560px){
        #reportsMetrics{grid-template-columns:1fr;}
        #reportsMetrics .metric-card.featured{grid-column:span 1;}
      }
    `;
